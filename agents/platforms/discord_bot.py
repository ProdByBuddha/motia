...
and a persistent jobs and health dashboard per server.
"""
...
# Make shared/modules importable when running from /opt
...
class DiscordAgentBot(commands.Bot):
    ...
    def setup_commands(self):
        ...
        async def session_cmd(ctx, action: str = None, *args):
            ...
            try:
                if action == "new" and args:
                    ...
                    return await ctx.send(f"✅ Created session `{sess['name']}` (id={sess['id']}) and set active")
                if action == "list":
        ...
        async def create_task(ctx, role: str, action: str, *, description: str):
            ...
            try:
                ...
                embed = discord.Embed(title="✅ Task Created", description=f"**Task ID**: `{task.task_id}`\n**Status**: {task.status}", color=discord.Color.green())
                ...
                if task.requires_approval:
                    embed.add_field(name="⚠️ Status", value="Pending approval", inline=False)
        ...
        async def approve_task(ctx, task_id: str):
            try:
                ...
                await ctx.send(f"✅ Task `{task_id}` approved and executing")
            except Exception as e:
                await ctx.send(f"❌ Error approving task: {e}")

        @self.command(name="status")
        async def task_status(ctx, task_id: str):
            try:
                status = await self.coordinator.get_task_status(task_id)
                embed = discord.Embed(title=f"📊 Task Status: {status['title']}", description=f"**ID**: `{status['task_id']}`", color=discord.Color.blue())
        ...
        async def pending_approvals(ctx):
            ...
            if not pending:
                ...
                return
            embed = discord.Embed(title="⏳ Pending Approvals", description=f"{len(pending)} tasks awaiting approval", color=discord.Color.orange())
        ...
        async def build_app(ctx, *, description: str):
            ...
            embed = discord.Embed(title="🏗️ Building Application", description="Agents are working on your project...", color=discord.Color.blue())
        ...
        async def talk_to_agent(ctx, role: str, *, message: str):
            try:
                ...
                if not agent:
                    await ctx.send(f"❌ Agent not found: {role}")
                    return
                agent.set_mode("conversational")
                response = await agent.run_conversational(message)
                embed = discord.Embed(title=f"💬 {agent_role.value.replace('_', ' ').title()}", description=response, color=discord.Color.purple())
                await ctx.send(embed=embed)
            except Exception as e:
        ...
        async def list_agents(ctx):
            embed = discord.Embed(title="🤖 Available Agents", description="Your autonomous dev team", color=discord.Color.green())
        ...
        async def help_command(ctx):
            embed = discord.Embed(title="🤖 Agent System Help", description="Autonomous dev agency at your command", color=discord.Color.blue())
        ...
        async def jobs_cmd(ctx, action: str = None, *args):
            guild = ctx.guild
            if not guild:
                return await ctx.send("❌ This must be run in a server channel.")
            if not action:
                return await ctx.send("Usage: !agent jobs [setup|refresh|show|logs] ...")
            try:
                if action == "setup":
                    if not args:
                    ...
                    if sub == "here":
                        ...
                        return await ctx.send("✅ Jobs dashboard bound to this channel.")
                    if sub == "channel" and len(args) >= 2:
                        ...
                        if ch_mention.startswith("<#") and ch_mention.endswith(">"):
                            try:
                                ...
                                pass
                        if not ch_id:
                            return await ctx.send("❌ Please mention a channel like #dashboard")
                        await self._set_jobs_dashboard(guild.id, ch_id)
                        return await ctx.send(f"✅ Jobs dashboard bound to <#{ch_id}>")
                    if sub == "create" and len(args) >= 2:
                        name = " ".join(args[1:])
                        ch = await guild.create_text_channel(name)
                        await self._set_jobs_dashboard(guild.id, ch.id)
                        return await ctx.send(f"✅ Created and bound dashboard channel {ch.mention}")
                    return await ctx.send("❌ Unknown setup subcommand")
                if action == "refresh":
                    await self.refresh_jobs_dashboard(guild.id)
                    return await ctx.send("🔄 Dashboard refreshed")
                if action == "show":
                    # Show jobs list immediately (no binding required)
                    import aiohttp
                    ...
                    try:
                        ...
                        return await ctx.send(f"❌ jobs fetch failed: {e}")
                    if not jobs:
                        return await ctx.send("No jobs yet.")
                    lines = []
                    for j in jobs[:15]:
                        ...
                        lines.append(f"`{jid}` • {kind} • {status} — {desc[:60]}")
                    desc = "\n".join(lines)
                    embed = discord.Embed(title="🗂️ Builder Jobs (now)", description=desc, color=discord.Color.blurple())
                    await ctx.send(embed=embed)
                    return
                if action == "logs" and args:
                    job_id = args[0]
                    import aiohttp
                    builder_url = os.getenv("BUILDER_API_URL", "http://127.0.0.1:4010")
                    try:
                        async with aiohttp.ClientSession() as s:
                            async with s.get(builder_url + f"/jobs/{job_id}", timeout=15) as resp:
                                job = await resp.json()
                    except Exception as e:
                        return await ctx.send(f"❌ fetch logs failed: {e}")
                    thread = None
                    try:
                        thread = await ctx.message.create_thread(name=f"Logs {job_id}")
                    except Exception:
                        thread = None
                    logs = ((job.get('result') or {}).get('logs')) or []
                    if not logs:
                        return await ctx.send("No logs available for this job.")
                    joined = "\n".join(str(x) for x in logs)
                    if len(joined) > 6000:
                        data = io.BytesIO(joined.encode('utf-8'))
                        if thread:
                            await thread.send(content="📎 Logs attached", file=discord.File(data, filename=f"logs_{job_id}.txt"))
                        else:
                            await ctx.send(content="📎 Logs attached", file=discord.File(data, filename=f"logs_{job_id}.txt"))
                        return
                    # else chunk
                    buf = []
                    current = ""
                    ...
                    for chunk in buf[:15]:
                        if thread:
                            await thread.send(f"```txt\n{chunk}\n```")
                        else:
                            await ctx.send(f"```txt\n{chunk}\n```")
                    return
                return await ctx.send("❌ Unknown jobs action")
            except discord.Forbidden:
                return await ctx.send("❌ Missing permissions to manage channels or send messages")
            except Exception as e:
                return await ctx.send(f"❌ {e}")
            ...
            try:
                if action == "setup":
                    ...
                    if sub == "here":
                        ...
                        return await ctx.send("✅ Health dashboard bound to this channel.")
                    if sub == "channel" and len(args) >= 2:
                        ch_mention = args[1]
                        ch_id = None
                        if ch_mention.startswith("<#") and ch_mention.endswith(">"):
                            try:
                                ...
                                pass
                        if not ch_id:
                            return await ctx.send("❌ Please mention a channel like #health")
                        await self._set_health_dashboard(guild.id, ch_id)
                        await self.refresh_health_dashboard(guild.id)
                        return await ctx.send(f"✅ Health dashboard bound to <#{ch_id}>")
                    return await ctx.send("❌ Unknown setup subcommand")
                if action == "refresh":
                    await self.refresh_health_dashboard(guild.id)
                    return await ctx.send("🔄 Health dashboard refreshed")
            except discord.Forbidden:
                return await ctx.send("❌ Missing permissions to manage channels or send messages")
            except Exception as e:
                return await ctx.send(f"❌ {e}")
    ...
    async def on_message(self, message):
        ...
        try:
            ...
            if isinstance(reply, dict):
                text_out = reply.get("text") or ""
                if text_out:
                    await message.channel.send(text_out)
                for notif in reply.get("notifications") or []:
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        if str(reaction.emoji) == "✅":
            ...
            await reaction.message.channel.send(f"✅ Task `{task_id}` approved by {user.mention}")
        elif str(reaction.emoji) == "❌":
            task = self.coordinator.tasks.get(task_id)
            if task:
                task.status = "rejected"
                await reaction.message.channel.send(f"❌ Task `{task_id}` rejected by {user.mention}")
    ...
    async def show_builder_notification(self, channel: discord.abc.Messageable, notif: dict, requester: Optional[discord.Member] = None):
        """Show interactive embed for builder jobs with optional Daytona logs preview."""
        try:
            ...
            if ntype != "builder_job" or not job_id:
                return
            title = {
                "propose_patch": "🔧 Proposed Patch Staged",
                "scaffold_service": "🧱 Service Scaffold Staged",
                "run_tests": "🧪 Test Run Result",
                "apply": "🚀 Apply Result",
                "rfc": "📝 RFC Draft",
            }.get(tool, "🔔 Builder Job")
            ...
            try:
                ...
                if tool == 'rfc':
                    ...
                    if rfc:
                        preview = (rfc[:500] + '…') if len(rfc) > 500 else rfc
                        if len(preview) > 1000:
                    ...
                    if logs:
                        ...
                        if len(preview) > 1000:
                            preview = preview[:1000] + '…'
                        embed.add_field(name="Logs (preview)", value=f"```txt\n{preview}\n```\nUse Logs for full output.", inline=False)
                elif tool == 'run_tests':
                    logs = ((job.get('result') or {}).get('logs')) or []
                    if logs:
                        preview_lines = []
            ...
            if tool == 'run_tests':
                view.add_item(self._LogsJobButton(self, job_id))
    ...
    async def refresh_jobs_dashboard(self, guild_id: int):
        ...
        embed = discord.Embed(title="🗂️ Builder Jobs", description=desc, color=discord.Color.blurple())
        ...
        for j in jobs[:5]:
            ...
            if status in ("tested", "error"):
                view.add_item(self._LogsJobButton(self, jid))
            # Add quick Logs button for tested/error jobs
            if status in ("tested", "error"):
                view.add_item(self._LogsJobButton(self, jid))
        # Upsert message
    ...
    class _ApproveJobButton(ui.Button):
        def __init__(self, bot: 'DiscordAgentBot', job_id: str):
            super().__init__(label=f"Approve {job_id[:6]}", style=discord.ButtonStyle.success, emoji="✅", custom_id=f"job:approve:{job_id}")
            self.bot = bot
            self.job_id = job_id
        async def callback(self, interaction: discord.Interaction):
            ...
            try:
                ...
                await interaction.followup.send(f"🚀 Applied `{self.job_id}`", ephemeral=True)
            except Exception as e:
    ...
    class _RejectJobButton(ui.Button):
        def __init__(self, bot: 'DiscordAgentBot', job_id: str):
            super().__init__(label=f"Reject {job_id[:6]}", style=discord.ButtonStyle.danger, emoji="🛑", custom_id=f"job:reject:{job_id}")
            self.bot = bot
            self.job_id = job_id
        async def callback(self, interaction: discord.Interaction):
            await interaction.response.defer(thinking=False)
            try:
                ...
                async with aiohttp.ClientSession() as s:
                    await s.post(url, json=payload, timeout=30)
                await interaction.followup.send(f"🛑 Rejected `{self.job_id}`", ephemeral=True)
            except Exception as e:
                await interaction.followup.send(f"❌ {e}", ephemeral=True)
            finally:
                await self.bot.refresh_jobs_dashboard(interaction.guild_id)
    ...
    class _DetailsJobButton(ui.Button):
        def __init__(self, bot: 'DiscordAgentBot', job_id: str):
            super().__init__(label=f"Details {job_id[:6]}", style=discord.ButtonStyle.secondary, emoji="📄", custom_id=f"job:details:{job_id}")
            self.bot = bot
            self.job_id = job_id
        async def callback(self, interaction: discord.Interaction):
            await interaction.response.defer(thinking=False)
            try:
                ...
                if rfc and thread:
                    try:
                        if len(rfc) > 6000:
                            data = io.BytesIO(rfc.encode('utf-8'))
                            await thread.send(content="📎 RFC attached", file=discord.File(data, filename=f"rfc_{self.job_id}.md"))
                        else:
                            for chunk in [rfc[i:i+1800] for i in range(0, len(rfc), 1800)]:
                                await thread.send(f"```md\n{chunk}\n```")
                    except Exception:
                        pass
                    await interaction.followup.send(f"📄 Posted RFC for `{self.job_id}`", ephemeral=True)
                    return
                # If test logs exist, post them chunked or as file
                logs = ((job.get('result') or {}).get('logs')) or []
                if logs and thread:
                    joined = "\n".join(str(x) for x in logs)
                    if len(joined) > 6000:
                        data = io.BytesIO(joined.encode('utf-8'))
                        await thread.send(content="📎 Logs attached", file=discord.File(data, filename=f"logs_{self.job_id}.txt"))
                    else:
                        buf = []
                        current = ""
                        for entry in logs:
                        ...
                        for chunk in buf[:10]:
                            await thread.send(f"```txt\n{chunk}\n```")
                    await interaction.followup.send(f"🧪 Posted logs for `{self.job_id}`", ephemeral=True)
                    return
                # If test logs exist, post them chunked
                logs = ((job.get('result') or {}).get('logs')) or []
                if logs and thread:
                    buf = []
                    current = ""
                ...
                if thread:
                    ...
                    await interaction.followup.send(f"📄 Opened thread for `{self.job_id}`", ephemeral=True)
                else:
                    await interaction.followup.send(f"```json\n{text[:1900]}\n```", ephemeral=True)
            except Exception as e:
                await interaction.followup.send(f"❌ Details failed: {e}", ephemeral=True)

    class _LogsJobButton(ui.Button):
        def __init__(self, bot: 'DiscordAgentBot', job_id: str):
            super().__init__(label=f"Logs {job_id[:6]}", style=discord.ButtonStyle.secondary, emoji="🧪", custom_id=f"job:logs:{job_id}")
            self.bot = bot
            self.job_id = job_id
        async def callback(self, interaction: discord.Interaction):
            await interaction.response.defer(thinking=False)
            try:
                ...
                thread = None
                try:
                    thread = await parent_msg.create_thread(name=f"Logs {self.job_id}")
                except Exception:
                    thread = None
                logs = ((job.get('result') or {}).get('logs')) or []
                if not logs:
                    return await interaction.followup.send("No logs available.", ephemeral=True)
                buf = []
                current = ""
                for entry in logs:
                    line = str(entry)
                ...
                for chunk in buf:
                    ...
                    if thread:
                        ...
                        await interaction.channel.send(f"```txt\n{chunk}\n```")
                    posted += 1
                await interaction.followup.send("🧪 Logs posted", ephemeral=True)
            except Exception as e:
                await interaction.followup.send(f"❌ Logs failed: {e}", ephemeral=True)

    class _RestartContainerButton(ui.Button):
        def __init__(self, bot: 'DiscordAgentBot', container: str):
            super().__init__(label=f"Restart {container[:12]}", style=discord.ButtonStyle.danger, emoji="♻️", custom_id=f"health:restart:{container}")
            self.bot = bot
            self.container = container
        async def callback(self, interaction: discord.Interaction):
            await interaction.response.defer(thinking=False)
            try:
                # Only allow restart if bot process has docker perms
                import subprocess
                subprocess.run(["docker", "restart", self.container], check=False, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, timeout=20)
                await interaction.followup.send(f"♻️ Restart requested for `{self.container}`", ephemeral=True)
            except Exception as e:
    ...
    class _HealthRefreshButton(ui.Button):
        def __init__(self, bot: 'DiscordAgentBot'):
            super().__init__(label="Refresh", style=discord.ButtonStyle.primary, emoji="🔄", custom_id="health:refresh")
            self.bot = bot
        async def callback(self, interaction: discord.Interaction):
            await interaction.response.defer(thinking=False)
            await self.bot.refresh_health_dashboard(interaction.guild_id)
    ...
    class _HealthViewAllButton(ui.Button):
        def __init__(self, bot: 'DiscordAgentBot'):
            super().__init__(label="View All", style=discord.ButtonStyle.secondary, emoji="📋", custom_id="health:viewall")
            self.bot = bot
        async def callback(self, interaction: discord.Interaction):
            await interaction.response.defer(thinking=False)
            try:
                ...
                thread = None
                try:
                    thread = await parent_msg.create_thread(name=f"Docker {datetime.utcnow().strftime('%H:%M:%S')} GMT")
                except Exception:
                    thread = None
                if not cons:
                    return await interaction.followup.send("No containers.", ephemeral=True)
                buf = []
                for c in cons:
                    stats = c.get('stats', {})
                    ln = f"• {c['name']} — {c['status']}\n"
                ...
                if buf:
                    if thread:
                        ...
                        await interaction.channel.send("\n".join(buf))
                await interaction.followup.send("📋 Posted container list", ephemeral=True)
            except Exception as e:
                await interaction.followup.send(f"❌ {e}", ephemeral=True)
