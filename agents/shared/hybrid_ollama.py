...
class HybridOllamaProvider:
    ...
    def _configure_backend(self):
        ...
        # Override with OLLAMA_BASE_URL if provided (remote GPU / custom endpoint)
        override = os.getenv("OLLAMA_BASE_URL")
        if override:
            self.base_url = override.rstrip("/")
            self.api_key = None
            self.timeout = int(self.custom_config.get("timeout", 30))
            self.latency_target = "remote"
            self.cost = "$0"
            return
